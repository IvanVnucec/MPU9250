...
static int MPU9250_writeAK8963Register(const MPU9250_Handle_s *MPU9250_Handle, uint8_t subAddress, uint8_t data) {

	// set slave 0 to the AK8963 and set for write
	if (MPU9250_Handle->MPU9250_writeRegister(I2C_SLV0_ADDR, AK8963_I2C_ADDR) < 0) {
		return -1;
	}
	// set the register to the desired AK8963 sub address
	if (MPU9250_Handle->MPU9250_writeRegister(I2C_SLV0_REG, subAddress) < 0) {
		return -2;
	}
	// store the data for write
	if (MPU9250_Handle->MPU9250_writeRegister(I2C_SLV0_DO, data) < 0) {
		return -3;
	}
	// enable I2C and send 1 byte
	if (MPU9250_Handle->MPU9250_writeRegister(I2C_SLV0_CTRL, I2C_SLV0_EN | (uint8_t) 1) < 0) {
		return -4;
	}
...
static int MPU9250_readAK8963Registers(const MPU9250_Handle_s *MPU9250_Handle, uint8_t subAddress, uint8_t count, uint8_t *dest) {

	// set slave 0 to the AK8963 and set for read
	if (MPU9250_Handle->MPU9250_writeRegister(I2C_SLV0_ADDR, AK8963_I2C_ADDR | I2C_READ_FLAG)
			< 0) {
		return -1;
	}
	// set the register to the desired AK8963 sub address
	...
	if (MPU9250_Handle->MPU9250_writeRegister(I2C_SLV0_CTRL, I2C_SLV0_EN | count) < 0) {
		return -3;
	}

	MPU9250_Handle->MPU9250_delayMiliSec(1); // takes some time for these registers to fill

	// read the bytes off the MPU9250 EXT_SENS_DATA registers
	return MPU9250_Handle->MPU9250_readRegisters(EXT_SENS_DATA_00, count, dest);
}


/* gets the MPU9250 WHO_AM_I register value, expected to be 0x71 */
static uint8_t MPU9250_whoAmI(const MPU9250_Handle_s *MPU9250_Handle) {
	uint8_t value;

	// read the WHO AM I register
	if (MPU9250_Handle->MPU9250_readRegisters(WHO_AM_I, 1, &value) < 0) {
		return -1;
	}
...
static uint8_t MPU9250_whoAmIAK8963(const MPU9250_Handle_s *MPU9250_Handle) {
	uint8_t value;

	// read the WHO AM I register
	if (MPU9250_readAK8963Registers(MPU9250_Handle, AK8963_WHO_AM_I, 1, &value) < 0) {
		return -1;
	}
	// return the register value
	return value;
...
int MPU9250_begin(MPU9250_writeRegister_T MPU9250_writeRegister,
	...
	if (MPU9250_Handle == NULL
		|| MPU9250_writeRegister == NULL
		|| MPU9250_readRegisters == NULL
		|| MPU9250_delayMiliSec  == NULL)
	{
		return -1; //failure
	}
	...
	// i2c
	MPU9250_Handle->MPU9250_writeRegister = MPU9250_writeRegister;
	MPU9250_Handle->MPU9250_readRegisters = MPU9250_readRegisters;
	
	// delay function
	MPU9250_Handle->MPU9250_delayMiliSec = MPU9250_delayMiliSec;
	...
	if (MPU9250_Handle->MPU9250_writeRegister(PWR_MGMNT_1, CLOCK_SEL_PLL) < 0) {
		return -1;
	}

	// enable I2C master mode
	if (MPU9250_Handle->MPU9250_writeRegister(USER_CTRL, I2C_MST_EN) < 0) {
		return -2;
	}

	// set the I2C bus speed to 400 kHz
	if (MPU9250_Handle->MPU9250_writeRegister(I2C_MST_CTRL, I2C_MST_CLK) < 0) {
		return -3;
	}

	// set AK8963 to Power Down
	MPU9250_writeAK8963Register(MPU9250_Handle, AK8963_CNTL1, AK8963_PWR_DOWN);

	// reset the MPU9250
	MPU9250_Handle->MPU9250_writeRegister(PWR_MGMNT_1, PWR_RESET);

	// wait for MPU-9250 to come back up
	MPU9250_Handle->MPU9250_delayMiliSec(1);

	// reset the AK8963
	MPU9250_writeAK8963Register(MPU9250_Handle, AK8963_CNTL2, AK8963_RESET);
	...
	if ((MPU9250_whoAmI(MPU9250_Handle) != 113) && (MPU9250_whoAmI(MPU9250_Handle) != 115)) {
		return -5;
	}

	// enable accelerometer and gyro
	if (MPU9250_Handle->MPU9250_writeRegister(PWR_MGMNT_2, SEN_ENABLE) < 0) {
		return -6;
	}
	...
	if (MPU9250_Handle->MPU9250_writeRegister(ACCEL_CONFIG, ACCEL_FS_SEL_16G) < 0) {
		return -7;
	}
	...
	if (MPU9250_Handle->MPU9250_writeRegister(GYRO_CONFIG, GYRO_FS_SEL_2000DPS) < 0) {
		return -8;
	}

	// setting bandwidth to 184Hz as default
	MPU9250_Handle->_bandwidth = DLPF_BANDWIDTH_184HZ;
	if (MPU9250_Handle->MPU9250_writeRegister(ACCEL_CONFIG2, ACCEL_DLPF_184) < 0) {
		return -9;
	}

	// setting gyro bandwidth to 184Hz
	if (MPU9250_Handle->MPU9250_writeRegister(CONFIG, GYRO_DLPF_184) < 0) {
		return -10;
	}

	// setting the sample rate divider to 0 as default
	MPU9250_Handle->_srd = 0;
	if (MPU9250_Handle->MPU9250_writeRegister(SMPDIV, 0x00) < 0) {
		return -11;
	}
	...
	// set AK8963 to Power Down
	if (MPU9250_writeAK8963Register(MPU9250_Handle, AK8963_CNTL1, AK8963_PWR_DOWN) < 0) {
		return -15;
	}

	MPU9250_Handle->MPU9250_delayMiliSec(100); // long wait between AK8963 mode changes

	// set AK8963 to FUSE ROM access
	if (MPU9250_writeAK8963Register(MPU9250_Handle, AK8963_CNTL1, AK8963_FUSE_ROM) < 0) {
		return -16;
	...
	MPU9250_readAK8963Registers(MPU9250_Handle, AK8963_ASA, 3, MPU9250_Handle->_buffer);
	...
	if (MPU9250_writeAK8963Register(MPU9250_Handle, AK8963_CNTL1, AK8963_CNT_MEAS2) < 0) {
		return -18;
	}
	...
	MPU9250_readAK8963Registers(MPU9250_Handle, AK8963_HXL, 7, MPU9250_Handle->_buffer);
...
int MPU9250_setAccelRange(AccelRange_t range, MPU9250_Handle_s *MPU9250_Handle) {
	switch (range) {
		case ACCEL_RANGE_2G: {
			...
			if (MPU9250_Handle->MPU9250_writeRegister(ACCEL_CONFIG, ACCEL_FS_SEL_2G) < 0) {
				return -1;
			}
		...
		case ACCEL_RANGE_4G: {
			...
			if (MPU9250_Handle->MPU9250_writeRegister(ACCEL_CONFIG, ACCEL_FS_SEL_4G) < 0) {
				return -1;
			}
		...
		case ACCEL_RANGE_8G: {
			...
			if (MPU9250_Handle->MPU9250_writeRegister(ACCEL_CONFIG, ACCEL_FS_SEL_8G) < 0) {
				return -1;
			}
...
int MPU9250_setGyroRange(GyroRange_t range, MPU9250_Handle_s *MPU9250_Handle) {
	switch (range) {
		case GYRO_RANGE_250DPS: {
			...
			if (MPU9250_Handle->MPU9250_writeRegister(GYRO_CONFIG, GYRO_FS_SEL_250DPS) < 0) {
				return -1;
			}
		...
		case GYRO_RANGE_500DPS: {
			...
			if (MPU9250_Handle->MPU9250_writeRegister(GYRO_CONFIG, GYRO_FS_SEL_500DPS) < 0) {
				return -1;
			}
		...
		case GYRO_RANGE_1000DPS: {
			...
			if (MPU9250_Handle->MPU9250_writeRegister(GYRO_CONFIG, GYRO_FS_SEL_1000DPS) < 0) {
				return -1;
			}
...
int MPU9250_setDlpfBandwidth(DlpfBandwidth_t bandwidth, MPU9250_Handle_s *MPU9250_Handle) {
	switch (bandwidth) {
		case DLPF_BANDWIDTH_184HZ: {
			if (MPU9250_Handle->MPU9250_writeRegister(ACCEL_CONFIG2, ACCEL_DLPF_184) < 0) { // setting accel bandwidth to 184Hz
				return -1;
			}
			if (MPU9250_Handle->MPU9250_writeRegister(CONFIG, GYRO_DLPF_184) < 0) { // setting gyro bandwidth to 184Hz
				return -2;
			}
			break;
		}
		case DLPF_BANDWIDTH_92HZ: {
			if (MPU9250_Handle->MPU9250_writeRegister(ACCEL_CONFIG2, ACCEL_DLPF_92) < 0) { // setting accel bandwidth to 92Hz
				return -1;
			}
			if (MPU9250_Handle->MPU9250_writeRegister(CONFIG, GYRO_DLPF_92) < 0) { // setting gyro bandwidth to 92Hz
				return -2;
			}
			break;
		}
		case DLPF_BANDWIDTH_41HZ: {
			if (MPU9250_Handle->MPU9250_writeRegister(ACCEL_CONFIG2, ACCEL_DLPF_41) < 0) { // setting accel bandwidth to 41Hz
				return -1;
			}
			if (MPU9250_Handle->MPU9250_writeRegister(CONFIG, GYRO_DLPF_41) < 0) { // setting gyro bandwidth to 41Hz
				return -2;
			}
			break;
		}
		case DLPF_BANDWIDTH_20HZ: {
			if (MPU9250_Handle->MPU9250_writeRegister(ACCEL_CONFIG2, ACCEL_DLPF_20) < 0) { // setting accel bandwidth to 20Hz
				return -1;
			}
			if (MPU9250_Handle->MPU9250_writeRegister(CONFIG, GYRO_DLPF_20) < 0) { // setting gyro bandwidth to 20Hz
				return -2;
			}
			break;
		}
		case DLPF_BANDWIDTH_10HZ: {
			if (MPU9250_Handle->MPU9250_writeRegister(ACCEL_CONFIG2, ACCEL_DLPF_10) < 0) { // setting accel bandwidth to 10Hz
				return -1;
			}
			if (MPU9250_Handle->MPU9250_writeRegister(CONFIG, GYRO_DLPF_10) < 0) { // setting gyro bandwidth to 10Hz
				return -2;
			}
			break;
		}
		case DLPF_BANDWIDTH_5HZ: {
			if (MPU9250_Handle->MPU9250_writeRegister(ACCEL_CONFIG2, ACCEL_DLPF_5) < 0) { // setting accel bandwidth to 5Hz
				return -1;
			}
			if (MPU9250_Handle->MPU9250_writeRegister(CONFIG, GYRO_DLPF_5) < 0) { // setting gyro bandwidth to 5Hz
				return -2;
			}
			break;
		}
...
int MPU9250_setSrd(uint8_t srd, MPU9250_Handle_s *MPU9250_Handle) {
	...
	if (MPU9250_Handle->MPU9250_writeRegister(SMPDIV, 19) < 0) { // setting the sample rate divider
		return -1;
	}
	...
	if (srd > 9) {
		...
		if (MPU9250_writeAK8963Register(MPU9250_Handle, AK8963_CNTL1, AK8963_CNT_MEAS1) < 0) {
			return -3;
		}
		MPU9250_Handle->MPU9250_delayMiliSec(100); // long wait between AK8963 mode changes
		// instruct the MPU9250 to get 7 bytes of data from the AK8963 at the sample rate
	...
	if (MPU9250_Handle->MPU9250_writeRegister(SMPDIV, srd) < 0) { // setting the sample rate divider
		return -4;
	}
...
int MPU9250_enableDataReadyInterrupt(MPU9250_Handle_s *MPU9250_Handle) {
	...
	if (MPU9250_Handle->MPU9250_writeRegister(INT_PIN_CFG, INT_PULSE_50US) < 0) { // setup interrupt, 50 us pulse
		return -1;
	}
	if (MPU9250_Handle->MPU9250_writeRegister(INT_ENABLE, INT_RAW_RDY_EN) < 0) { // set to data ready
		return -2;
	}
...
int MPU9250_disableDataReadyInterrupt(MPU9250_Handle_s *MPU9250_Handle) {
	if (MPU9250_Handle->MPU9250_writeRegister(INT_ENABLE, INT_DISABLE) < 0) { // disable interrupt
		return -1;
	}
...
int MPU9250_enableWakeOnMotion(float womThresh_mg, LpAccelOdr_t odr, MPU9250_Handle_s *MPU9250_Handle) {
	...
	// wait for MPU-9250 to come back up
	MPU9250_Handle->MPU9250_delayMiliSec(1);
	if (MPU9250_Handle->MPU9250_writeRegister(PWR_MGMNT_1, 0x00) < 0) { // cycle 0, sleep 0, standby 0
		return -1;
	}
	if (MPU9250_Handle->MPU9250_writeRegister(PWR_MGMNT_2, DIS_GYRO) < 0) { // disable gyro measurements
		return -2;
	}
	if (MPU9250_Handle->MPU9250_writeRegister(ACCEL_CONFIG2, ACCEL_DLPF_184) < 0) { // setting accel bandwidth to 184Hz
		return -3;
	}
	if (MPU9250_Handle->MPU9250_writeRegister(INT_ENABLE, INT_WOM_EN) < 0) { // enabling interrupt to wake on motion
		return -4;
	}
	if (MPU9250_Handle->MPU9250_writeRegister(MOT_DETECT_CTRL,
	...
	if (MPU9250_Handle->MPU9250_writeRegister(WOM_THR, MPU9250_Handle->_womThreshold) < 0) { // setting wake on motion threshold
		return -6;
	}
	if (MPU9250_Handle->MPU9250_writeRegister(LP_ACCEL_ODR, (uint8_t) odr) < 0) { // set frequency of wakeup
		return -7;
	}
	if (MPU9250_Handle->MPU9250_writeRegister(PWR_MGMNT_1, PWR_CYCLE) < 0) { // switch to accel low power mode
		return -8;
	}
	return 1;
}
...
int MPU9250FIFO_enableFifo(uint32_t accel, uint32_t gyro, uint32_t mag, uint32_t temp, 
	...
	if (MPU9250_Handle->MPU9250_writeRegister(USER_CTRL, (0x40 | I2C_MST_EN)) < 0) {
		return -1;
	}

	if (MPU9250_Handle->MPU9250_writeRegister(FIFO_EN,
			(accel * FIFO_ACCEL)	|
			(gyro  * FIFO_GYRO) 	|
			(mag   * FIFO_MAG) 		|
			(temp  * FIFO_TEMP)	) < 0) {
...
int MPU9250_readSensor(MPU9250_Handle_s *MPU9250_Handle) {
	...
	if (MPU9250_Handle->MPU9250_readRegisters(ACCEL_OUT, 21, MPU9250_Handle->_buffer) < 0) {
		return -1;
	}
...
int MPU9250FIFO_readFifo(MPU9250_Handle_s *MPU9250_Handle) {
	...
	// get the fifo size
	MPU9250_Handle->MPU9250_readRegisters(FIFO_COUNT, 2, MPU9250_Handle->_buffer);
	...
	for (uint32_t i = 0; i < MPU9250_Handle->_fifoSize / MPU9250_Handle->_fifoFrameSize; i++) {
		// grab the data from the MPU9250
		if (MPU9250_Handle->MPU9250_readRegisters(FIFO_READ, MPU9250_Handle->_fifoFrameSize, MPU9250_Handle->_buffer) < 0) {
			return -1;
		}
...
int MPU9250_calibrateGyro(MPU9250_Handle_s *MPU9250_Handle) {
	...
	for (uint32_t i = 0; i < MPU9250_Handle->_numSamples; i++) {
		...
		MPU9250_Handle->MPU9250_delayMiliSec(20);
	}
